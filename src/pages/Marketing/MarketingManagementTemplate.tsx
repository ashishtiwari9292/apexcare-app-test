...
import TemplateModal from 'views/CareManagerActivities/TemplateModal';
import TemplateModalContent from 'views/CareManagerActivities/TemplateModalContent';

export function MarketingManagementTemplate({ filter, data, type, setActivities }: SectionProps) {
    ...
    const fetchData = useCallback(() => {
        setLoading(true);
        setRows([]);
        const url = '/management-activity-template'
    ...
    const handleSort = (sortVal: string, type: string, ascending: boolean) => {
        setRows(careManagerSort(rows, sortVal, type, ascending, 'careManagerActivities', true,filter.groupBy))
    }
    ...
    const confirmHandler = () => {
        ...
        API.post(`/management-activity-template/activity`, { _id: selectedRow.state._id, idx: selectedRow.state.idx })
            .then((rsp) => {
                toast.dismiss();
                if (rsp.data.success) {
    ...
    const confirmRemoveHandler = () => {
        ...
        API.post(`/management-activity-template/remove`, { _id: selectedRow.data[0].state._id })
            .then((rsp) => {
                toast.dismiss();
                if (rsp.data.success) {
    ...
    useEffect(() => {
        API.get('management-activity-template/new hire')
            .then(rsp => {
    ...
    return (
        <Card>
            <CardHeader
                title="Marketing Task Templates"
                setOpenModal={setOpenAddModal}
                expanded={expanded}
                setExpanded={setExpanded}
            />
            <Modal open={openAddModal} closeHandler={handleCloseAddModal} title=" Add Template"  >
                <TemplateModal closeHandler={handleCloseAddModal} management = {true} />
            </Modal >
            ...
            {!loading && rows.length > 0 && (
                <CardContent expanded={expanded}>
                    <Modal open={editOpenModal} closeHandler={editHandleCloseModal} title="Edit Template">
                        <TemplateModalContent closeHandler={editHandleCloseModal} selected={selectedRow}  management = {true}/>
                    </Modal>
                    <Modal open={editTemplateModal} closeHandler={editTemplateCloseModal} title=" Edit Template" >
                        <TemplateModal closeHandler={editTemplateCloseModal} data={selectedRow} management = {true}/>
                    </Modal >
