...
const ProspectsActivityModal = ({ closeMe, currentRow, options }: editApplicantsProps) => {
  const { users } = useCompany();
  const { user } = useAuth();
  const {prospectId} = useParams();

  const editProspectActivity = async (values: any) => {
    API.put('prospects/activities/' + currentRow.id, { ...values, modifiedBy: user.id })
      .then((rsp) => {
        if (rsp.data.success) {
          toast.success('Successfully Edited Prospect Activity!');
          closeMe();
        }
      })
      .catch((err) => {
        toast.error('Failed to edit Prospect Activity.');
        console.error(err);
       });
  };

  const createProspectActivity = (values: any) => {
    API.post('/prospects/activities', { ...values, prospect:prospectId, createdBy: user.id })
      .then((rsp) => {
        if (rsp.data.success) {
          toast.success('Successfully Added Prospect Activity.');
          closeMe();
        }
      })
      .catch((error) => {
        toast.error('Failed to add Prospect Activity.');
        console.error(error);
      });
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting }) => {
      currentRow ? editProspectActivity(values) : createProspectActivity(values);
      closeMe();
    },
  });
  useEffect(()=>{
...
export default ProspectsActivityModal;
