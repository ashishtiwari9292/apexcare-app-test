...
const MarketingActivitiesModalContent = ({ source, renderCompletedDate = false, stepper = false, renderType, renderPartners = true, closeHandler, currentRow, selected, showType, data, addActivity, vals = [], batchAdd, renderButtons, setVals, activeStep = 0, steps, handleNext, handleBack, prefill, submitTemplate, completed, activity = false, activityTypes, activityType, currentLocation }: any): JSX.Element => {
  ...
  let initialValues: any = activity ? {
    date: currentRow ? formatCMDate(currentRow?.state?.currentRow?.date) : Date.now(),
    time: currentRow?.state?.currentRow?.time || '',
    completedDate: currentRow ? formatCMDate(currentRow?.state?.currentRow?.completedDate) : null,
    completedTime: "",
    location: currentRow?.state?.currentRow?.location || user?.location,
    ...
    additionalComments: currentRow?.comments?.value,
    description: currentRow?.description?.value || currentRow?.state?.currentRow?.description || '',
    finalComments: currentRow?.state?.currentRow?.finalComments,
    recurrence: currentRow?.state?.recurrence?.type || 'No Recurrence',
    recurrenceOptions: currentRow?.state?.recurrence?.options,
    days: currentRow?.state?.recurrence?.days,
    prospect: currentRow?.prospect?.state?.fullName || currentRow?.state?.value?.prospect?.fullName || currentRow?.prospect?.selected?.fullName || formatName(selected?.state?.value?.prospect.firstName, selected?.state?.value?.prospect.lastName) || ''
  } : {
    flag: selected ? selected?.flag?.selected || false : false,
    location: currentLocation || selected?.state?.value?.location || user?.location,
    careManager: selected ? selected?.state?.value?.createdBy || selected?.state.value.careManager || selected?.state?.value?.marketingManager : (vals[activeStep]?.careManager?.firstName || vals[activeStep]?.careManager?.lastName) ? { firstName: vals[activeStep]?.careManager?.firstName, lastName: vals[activeStep]?.careManager?.lastName } : undefined,
    ...
    carePartner: selected ? selected?.state?.value?.carePartnerObj : { companyName: vals[activeStep]?.carePartner.companyName || '', _id: vals[activeStep]?.carePartner._id },
    date: selected ? !activity ? source === 'marketingCalendar' ? selected?.date?.value || null : selected?.dueDate?.value || null : selected?.date?.value || null : activeStep > 0 ? dateCalulator(prefill?.dateCalculator, vals[activeStep - 1]?.date) || null : vals[activeStep]?.date || null,
    time: selected?.time?.raw ? formatTimeString(selected?.time?.value || '') || '' : vals[activeStep]?.time || '',
    completedDate: selected ? formatCMDate(selected?.state?.value?.completedDate) : null,
    completedTime: selected?.state?.value?.completedTime || '',
    activity: selected ? selected?.state?.value.activity?._id || vals[activeStep]?.activity._id : vals[activeStep]?.activity._id || '',
    additionalComments: selected ? selected?.additionalComments?.value || '' : vals[activeStep]?.additionalComments || '',
    ...
    description: selected ? selected?.state?.value?.description || vals[activeStep]?.description : selected?.state?.currentRow?.description || vals[activeStep]?.description || '',
    finalComments: selected ? selected?.state?.value?.finalComments || selected.finalComments : '',
    prospect: currentRow?.prospect?.state?.fullName || selected?.state?.value?.prospect?.fullName || vals[activeStep]?.prospect || formatName(selected?.state?.value?.prospect?.firstName, selected?.state?.value?.prospect?.lastName) || ''
  };
  ...
  useEffect(() => {
    if (formik.values.careManager === undefined && user?.firstName && user?.lastName) {
    ...
    <form onSubmit={formik.handleSubmit}>
      <Stack>
        ...
        >
          ...
          {(completed === 'Prospect' || source === 'prospects-detail' || source === undefined || source === 'Batch' || source === 'marketingCalendar' || activityType === 'Prospect') && !renderP   ?
            <>
...
export default MarketingActivitiesModalContent;