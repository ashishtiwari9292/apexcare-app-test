...
const MarketingCalendarTasksModal = ({ source, renderCompletedDate = false, stepper = false, renderType, renderPartners = true, closeHandler, currentRow, selected, showType, data, addActivity, vals = [], batchAdd, renderButtons, setVals, activeStep = 0, steps, handleNext, handleBack, prefill, submitTemplate, completed, activity = false, activityTypes, activityType, currentLocation }: any): JSX.Element => {
  ...
  let initialValues: any ={
    flag: selected?.flag?.selected || false,
    location: selected?.state?.value?.location || user?.location,
    careManager: selected?.state?.value?.createdBy || selected?.state.value.careManager || selected?.state?.value?.marketingManager,
    client: selected?.state?.value?.clientObj,
    carePartner: selected?.state?.value?.carePartnerObj,
    date: selected?.date?.value || null,
    time: selected?.time?.raw ? formatTimeString(selected?.time?.value || null) : null,
    completedDate: formatCMDate(selected?.state?.value?.completedDate),
    completedTime: selected?.state?.value?.completedTime || '',
    activity: selected?.state?.value?.activity?._id,
    additionalComments: selected?.additionalComments?.value || '',
    recurrence: selected?.state?.value?.recurrence?.type || 'No Recurrence',
    recurrenceOptions: selected?.state?.value?.recurrence?.options,
    days: selected?.state?.value?.recurrence?.days || [],
    endDate: selected?.state?.value?.recurrence?.endDate || yearInTheFuture,
    description: selected?.state?.value?.description || '',
    finalComments: selected?.state?.value?.finalComments || '',
    prospect: selected?.state?.value?.prospect?.fullName || formatName(selected?.state?.value?.prospect?.firstName, selected?.state?.value?.prospect?.lastName) || ''
  }
...
export default MarketingCalendarTasksModal;