...
const MarketingTasksModalContent = ({ source, renderCompletedDate = false, stepper = false, renderType, renderPartners = true, closeHandler, currentRow, selected, showType, data, addActivity, vals = [], batchAdd, renderButtons, setVals, activeStep = 0, steps, handleNext, handleBack, prefill, submitTemplate, completed, activity = false, activityTypes, activityType, currentLocation }: any): JSX.Element => {
  ...
  let initialValues: any ={
    ...
    location: selected?.state?.value?.location,
    careManager: selected?.state?.value?.careManager || selected?.state?.value?.createdBy,
    client: selected?.state?.value?.clientObj,
    carePartner: selected?.state?.value?.carePartnerObj,
    date: formatCMDate(selected?.state?.value?.date),
    time: selected?.time?.value ? formatTimeString(selected?.time?.value || '') : '',
    ...
    prospect: selected?.state?.value?.prospect?.fullName
  }
...
export default MarketingTasksModalContent;